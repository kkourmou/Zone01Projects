...
- Allowed packages (stdlib only): PASS (Go code imports standard library + local `ascii-art/internal/...` packages only).
- Dockerfile exists: PASS (`Dockerfile` at repo root).
- Builds an image: PASS (`docker build -t ascii-art-web .` or `pwsh scripts/docker.ps1 build`).
- Runs a container: PASS (`docker run --detach --name ascii-art-web -p 8080:8080 ascii-art-web` or scripts).
- Runs the web server: PASS (listens on `:8080` by default; configurable via `PORT` env var).
- Container filesystem (expected): PASS (should contain `/app/ascii-art-web`, `/app/static`, `/app/templates`, `/app/ascii-style`).
- Metadata on Docker objects:
  - Image metadata: PASS (OCI labels `org.opencontainers.image.*` in `Dockerfile`; values can be injected via `BUILD_DATE`, `VCS_REF`, `SOURCE_URL` build args).
  - Container metadata: PASS (labels applied via `docker run --label ...` and `scripts/docker.*`).
- Garbage collection / unused objects: PASS (commands documented in `README.md`; `scripts/docker.* clean` prunes unused containers/images/networks).
- Dockerfile best practices: PASS (multi-stage build, minimal runtime `scratch`, non-root `USER 65532:65532`, `.dockerignore`, `ENTRYPOINT`).
